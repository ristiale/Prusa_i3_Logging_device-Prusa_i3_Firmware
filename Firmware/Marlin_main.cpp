...
extern void show_user_name_after_print();
...
extern bool print_successful; // PrusaLab
/*#FLB*/
  ...
  int8_t SilentMode;
    ...
    */
    if (code_seen_P(PSTR("M117"))) //moved to highest priority place to be able to to print strings which includes "G", "PRUSA" and "^"
    {
      ...
      strncpy(input_data, (strchr_pointer + /*11*/5), array_size); // PrusaLab
      ...
      // PrusaLab
      // There are two types of modes 'member' - print for members, 'master' - prints for custom-made orders
      // If first two characters are M:
      else if((input_data[0] == 'M') && (input_data[1] == ':')){
        for(int i=0; i<=5; i++) {
          FM_Mode[i] = input_data[i+2];
        }
        FM_Mode[6] = '\0';
        SERIAL_ECHO("UNM Mode: ");
        SERIAL_ECHOLN(FM_Mode);
      }
	...
	*/
	case 115: // M115
      if (code_seen('V')) {
          ...
          // PrusaLab upgrade dialog disabled
