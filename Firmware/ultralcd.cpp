...
void lcd_FM_login_screen() {
  ...
  lcd_update_enable(false);
  lcd_clear();
  //lcd_set_custom_characters_progress(); updated
  
  lcd_puts_at_P(1, 1, _i("Prusa i3 PrusaLab"));  // PrusaLab
...
void lcd_FM_offline_screen() {
  ...
  lcd_puts_P(PSTR(ESC_2J ESC_H(1, 1) "Prusa i3 PrusaLab" ESC_H(3, 2) "Swipe to login")); //PrusaLab
}
...
static void menu_action_sdfile(const char* filename)
{
  ...
  //PrusaLab - Is user working on a project in database??
  if (result)
  {
	if((FM_Mode[0] == 'm') && (FM_Mode[1] == 'a') && (FM_Mode[2] == 's')) //is user master?
	{
		project = lcd_show_fullscreen_message_yes_no_and_wait_P(_i("Tisk zakazky, ktera je v databazi?"), false, false); // Prusalab
		if(project){ // If user selects Yes
			SERIAL_ECHOLN("ESP posli probihajici zakazky k zobrazeni"); // NEEDS MODIFICATION **********************************************************
		} 
		lcd_update_enable(true);
	}
  }
  file_check = false; // Prusalab (has to be addded due to print ok message error)
  lcd_return_to_status();
}
